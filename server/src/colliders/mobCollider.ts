...
export class mobCollider {
  ...
  do(room, bodyA, bodyB) {
    ...
    if (bodyA.isMob && !bodyA.dead && bodyB.isPlayer) {


console.log('mob hits player');
      ...
      bodyB.struck = true;
      const promise1 = Promise.resolve(playerModel.updatePlayerStats(bodyB.discordName, 'health', -10, false));
      promise1.then(() => {
        bodyB.health = bodyB.health - 10;

        if (bodyB.health <= 0) {
          //bodyB.health = 0;
          const promise1 = Promise.resolve(playerModel.updatePlayerStats(bodyB.discordName, 'health', 80, true));
